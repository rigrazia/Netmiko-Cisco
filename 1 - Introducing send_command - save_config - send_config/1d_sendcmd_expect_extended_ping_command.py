...
for device in devices:
    try:
        ...
        # Construct the extended ping command
        extended_ping_command = 'ping'
        extended_ping_command += ' 192.168.1.1'  # Target IP address
        extended_ping_command += ' source GigabitEthernet0/0'  # Source interface

        # Send the extended ping command and expect prompts
        output = connection.send_command_expect(extended_ping_command)
        print(f"Command: {extended_ping_command}\nOutput:\n{output}\n")
